            ...
            arrTasks.push(function(callback){
                ...
                if(mobile){
                    ...
                    switch(cmd){
                        ...
                        case 'setup':
                            arrCodes = [];
                            arrCodes.push('var cmdStr = \'python setupTables.py\';');
                            arrCodes.push('var exec = require(\'child_process\').execSync;');
                            arrCodes.push('var rc = exec(cmdStr).toLocaleLowerCase();');
                            arrCodes.push('var code;');
                            arrCodes.push('console.log(rc.toString("utf8").trim());');
                            arrCodes.push('if(rc.includes(\'failed to \')){');
                            arrCodes.push('    code=1;');
                            arrCodes.push('}');
                            arrCodes.push('else{');
                            arrCodes.push('    code=0;');
                            arrCodes.push('}');
                            arrCodes.push('code.should.equal(0);');
                            pushTestCode('setup', '', data, arrCodes);;
                            checkerBrowser && checkerBrowser.sleep(data).then(doNext).catch(catchError) || doNext();
                            // checkerBrowser && checkerBrowser.sleep(data).then(doNext).catch(catchError) || doNext();
                            break;
                        // add by 61:
                        case 'export file expect':
                            var configFile = data.configFile;
                            var configSection = data.configSection;
                            var expectType = data.expectType;
                            if(expectType === 'Text File'){
                                arrCodes = [];
                                arrCodes.push('var cmdStr = \'python compareFile.py ' + configFile + ' ' + configSection + '\';');
                                arrCodes.push('var exec = require(\'child_process\').execSync;');
                                arrCodes.push('var rc = exec(cmdStr).toLocaleLowerCase();');
                                arrCodes.push('var code;');
                                arrCodes.push('console.log(rc.toString("utf8").trim());');
                                arrCodes.push('if(rc.includes(\'compare assert failed\')){');
                                arrCodes.push('    code=1;');
                                arrCodes.push('}');
                                arrCodes.push('else{');
                                arrCodes.push('    code=0;');
                            ...
                            if(expectType === 'Excel'){
                                arrCodes = [];
                                arrCodes.push('var cmdStr = \'python compareExcel.py ' + configFile + ' ' + configSection + '\';');
                                arrCodes.push('var exec = require(\'child_process\').execSync;');
                                arrCodes.push('var rc = exec(cmdStr).toLocaleLowerCase();');
                                arrCodes.push('var code;');
                                arrCodes.push('console.log(rc.toString("utf8").trim());');
                            ...
                            if(expectType === 'Zip File'){
                                arrCodes = [];
                                arrCodes.push('var cmdStr = \'python compareZip.py ' + configFile + ' ' + configSection + '\';');
                                arrCodes.push('var exec = require(\'child_process\').execSync;');
                                arrCodes.push('var rc = exec(cmdStr).toLocaleLowerCase();');
                                arrCodes.push('var code;');
                                arrCodes.push('console.log(rc.toString("utf8").trim());');
                            ...
                            pushTestCode('export file expect', '', data.testName, arrCodes);;
                            checkerBrowser && checkerBrowser.sleep(1).then(doNext).catch(catchError) || doNext();
                            // checkerBrowser && checkerBrowser.sleep(data).then(doNext).catch(catchError) || doNext();
                            break;
                        case 'waitBody':
                            arrCodes = [];
        ...
        function onReady(serverPort){
            ...
            setTimeout(function(){
                newChromeBrowser({wdPort: wdPort, hosts: hosts, wdPacUrl: wdPacUrl, isRecorder: true, debug: debug }, function*(browser){
                    ...
                    yield browser.url('chrome-extension://' + (debug ? 'fcndoegnpdfojfepgdkjcmapkgmfiphp' : 'njkfhfdkecbpjlnfmminhmdcakopmcnc')+'/'+(mobile?'mobile':'start')+'.html?port='+serverPort);
